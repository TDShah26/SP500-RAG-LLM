...
# Read the S&P 500 company list from Wikipedia into a pandas DataFrame
sp500 = pd.read_html('https://en.wikipedia.org/wiki/List_of_S%26P_500_companies')[0]

# Replace dots with hyphens in the 'Symbol' column to match Yahoo Finance ticker format
sp500['Symbol'] = sp500['Symbol'].str.replace('.', '-')

# Create a list of unique ticker symbols from the 'Symbol' column
symbols_list = sp500['Symbol'].unique().tolist()

sp_500_tickers = symbols_list
# Initialize a DataFrame to hold all predictions
all_predictions = pd.DataFrame()

for ticker in sp_500_tickers:
    # Download stock data
    stock_data = yf.download(ticker, period='6mo', interval='1d')  # assuming you want daily predictions
